...
int at_server_init(void)
{
    ...
    if (at_server_local->device)
    {
        ...
        at_device_set_rx_indicate(at_server_local->device, at_rx_ind);
    }
    else
    {
    ...
    at_server_local->parser = at_osal_thread_create("at_svr",
...
__exit:
    if (!result)
    {
        ...
        at_osal_thread_startup(at_server_local->parser);
