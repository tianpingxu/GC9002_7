...
__STATIC QRRawCode *QRraw_new(QRinput *input)
{
	...
	raw = (QRRawCode *)qr_malloc(sizeof(QRRawCode));
	...
	raw->ecccode = (unsigned char *)qr_malloc(raw->eccLength);
	if(raw->ecccode == NULL) {
		qr_free(raw->datacode);
		qr_free(raw);
		return NULL;
	}

	raw->blocks = QRspec_rsBlockNum(spec);
	raw->rsblock = (RSblock *)qr_calloc(raw->blocks, sizeof(RSblock));
	if(raw->rsblock == NULL) {
		QRraw_free(raw);
		return NULL;
	}
...
__STATIC MQRRawCode *MQRraw_new(QRinput *input)
{
	...
	raw = (MQRRawCode *)qr_malloc(sizeof(MQRRawCode));
	if(raw == NULL) return NULL;
	...
	raw->rsblock = (RSblock *)qr_calloc(1, sizeof(RSblock));
	if(raw->rsblock == NULL) {
		MQRraw_free(raw);
		return NULL;
	}
...
static FrameFiller *FrameFiller_new(int width, unsigned char *frame, int mqr)
{
	...
	filler = (FrameFiller *)qr_malloc(sizeof(FrameFiller));
...
__STATIC QRcode *QRcode_new(int version, int width, unsigned char *data)
{
	...
	qrcode = (QRcode *)qr_malloc(sizeof(QRcode));
...
__STATIC QRcode *QRcode_encodeMask(QRinput *input, int mask)
{
	...
	if(mask == -2) { // just for debug purpose
		masked = (unsigned char *)qr_malloc(width * width);
...
static QRcode_List *QRcode_List_newEntry(void)
{
	...
	entry = (QRcode_List *)qr_malloc(sizeof(QRcode_List));
