 ...
 *      端口号任意填写
 **/
#define TEST_UDP_REMOTE_IP      "192.168.3.13"
#define TEST_UDP_REMOTE_PORT    10013
static void task1(void *p)
{
    ...
    while(1)
    {
        ...
        while(1)
        {
            result = at_tcpip_CIPSTART_udp(dev, 0, TEST_UDP_REMOTE_IP, TEST_UDP_REMOTE_PORT, 0, 0);
            if(RT_EOK == result)
            {
                break;
            }
        ...
        result = at_tcpip_CIPSEND(dev, 0, str, strlen(str), 1, TEST_UDP_REMOTE_IP, TEST_UDP_REMOTE_PORT);
        if(RT_EOK != result)
        {
            csi_kernel_delay(pdMS_TO_TICKS(1000));
        }
