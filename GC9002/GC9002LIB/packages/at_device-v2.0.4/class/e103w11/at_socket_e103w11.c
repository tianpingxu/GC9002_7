...
#define LOG_TAG                       "at.skt.ecr"
#include <at_log.h>

#if(defined(AT_DEVICE_USING_E103W11) && (AT_DEVICE_USING_E103W11 == 1U) && defined(AT_USING_SOCKET) && (AT_USING_SOCKET == 1U))
...
#define E103W11_EVENT_CONN_OK          (1L << 0)
#define E103W11_EVENT_SEND_OK          (1L << 1)
#define E103W11_EVENT_RECV_OK          (1L << 2)
#define E103W11_EVNET_CLOSE_OK         (1L << 3)
#define E103W11_EVENT_CONN_FAIL        (1L << 4)
#define E103W11_EVENT_SEND_FAIL        (1L << 5)
...
static int e103w11_socket_event_send(struct at_device *device, uint32_t event)
{
    return (int) at_osal_event_send(device->socket_event, event);
}

static int e103w11_socket_event_recv(struct at_device *device, uint32_t event, uint32_t timeout, rt_uint8_t option)
{
    int result = 0;
    rt_uint32_t recved;
 ...
 *         -5: no memory
 */
static int e103w11_socket_close(struct at_socket *socket)
{
    int result = RT_EOK;
 ...
 *          -5: no memory
 */
static int e103w11_socket_connect(struct at_socket *socket, char *ip, int32_t port, enum at_socket_type type, rt_bool_t is_client)
{
    int result = RT_EOK;
...
__retry:
    ...
    if (result != RT_EOK && retryed == RT_FALSE)
    {
        ...
        if (e103w11_socket_close(socket) < 0)
        {
            goto __exit;
        }
        retryed = RT_TRUE;
 ...
 *          -5: no memory
 */
static int e103w11_socket_send(struct at_socket *socket, const char *buff, size_t bfsz, enum at_socket_type type)
{
    int result = RT_EOK;
    int event_result = 0;
    ...
    at_osal_mutex_take(lock, RT_WAITING_FOREVER);

    /* set current socket for send URC event */
    e103w11->user_data = (void *) device_socket;
    ...
    {
        if (bfsz - sent_size < E103W11_MODULE_SEND_MAX_SIZE)
        {
        ...
        if (e103w11_socket_event_recv(device, SET_EVENT(device_socket, 0),
        ...
        event_result = e103w11_socket_event_recv(device, E103W11_EVENT_SEND_OK | E103W11_EVENT_SEND_FAIL,
        ...
        /* check result */
        if (event_result & E103W11_EVENT_SEND_FAIL)
        {
 ...
 *         -5: no memory
 */
static int e103w11_domain_resolve(const char *name, char ip[16])
{
 ...
 */
static void e103w11_socket_set_event_cb(at_socket_evt_t event, at_evt_cb_t cb)
{
...
static const struct at_socket_ops e103w11_socket_ops =
{
    e103w11_socket_connect,
    e103w11_socket_close,
    e103w11_socket_send,
    e103w11_domain_resolve,
    e103w11_socket_set_event_cb,
...
static void urc_send_func(struct at_client *client, const char *data, rt_size_t size)
{
    int device_socket = 0;
    struct at_device *device = RT_NULL;
    struct at_device_e103w11 *e103w11 = RT_NULL;
    ...
    e103w11 = (struct at_device_e103w11 *) device->user_data;
    device_socket = (int) e103w11->user_data;

    if (rt_strstr(data, "SEND OK"))
    {
        e103w11_socket_event_send(device, SET_EVENT(device_socket, E103W11_EVENT_SEND_OK));
    }
    else if (rt_strstr(data, "SEND FAIL"))
    {
        e103w11_socket_event_send(device, SET_EVENT(device_socket, E103W11_EVENT_SEND_FAIL));
    }
}
...
int e103w11_socket_class_register(struct at_device_class *class)
{
    ...
    class->socket_num = AT_DEVICE_E103W11_SOCKETS_NUM;
    class->socket_ops = &e103w11_socket_ops;

    return RT_EOK;
}

#endif /* AT_DEVICE_USING_E103W11 && AT_USING_SOCKET */
