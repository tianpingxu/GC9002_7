...
static bool usbd_set_endpoint(const struct usb_endpoint_descriptor *ep_desc)
{
	...
	bool status = usbd_ep_open(&ep_cfg) == 0;
    USB_LOG_INFO("Open endpoint:0x%x ep_idx :0x%x type:%u mps:%u status :%d\r\n",
                 ep_cfg.ep_addr, USB_EP_GET_IDX(ep_cfg.ep_addr), ep_cfg.ep_type, ep_cfg.ep_mps,status);
...
{
	printf("set_configuration conifg_index:%d,alt_settings:%d\n",config_index,alt_setting);
...
//		printf("USB_REQUEST_GET_DESCRIPTOR: value: %d\n",value);
...
static int usbd_standard_request_handler(struct usb_setup_packet *setup, uint8_t **data, uint32_t *len)
{
    ...
    switch (setup->bmRequestType & USB_REQUEST_RECIPIENT_MASK) {
		...
		//printf("USB_REQUEST_RECIPIENT_DEVICE\n");
		...
		printf("USB_REQUEST_RECIPIENT_INTERFACE\n");
		...
		printf("USB_REQUEST_RECIPIENT_ENDPOINT\n");
...
#endif
//	printf("Send data or status to host\n");
