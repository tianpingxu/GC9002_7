...
typedef struct {
    __IOM uint32_t CFG;           /*!< Offset: 0x000 (R/W)  CLIC configure register */
    __IM  uint32_t INFO;          /*!< Offset: 0x004 (R/ )  CLIC information register */
    __IOM uint32_t MTH;           /*!< Offset: 0x00B (R/W)  CLIC machine mode threshold register */
...
#define CLIC_CLICCFG_NVBIT_Pos          0U                                      /*!< CLIC CFG: NVBIT Position */
#define CLIC_CLICCFG_NVBIT_Msk          (0x1UL << CLIC_CLICCFG_NVBIT_Pos)       /*!< CLIC CFG: NVBIT Mask */

#define CLIC_CLICCFG_NLBIT_Pos          1U                                      /*!< CLIC CFG: NLBIT Position */
#define CLIC_CLICCFG_NLBIT_Msk          (0xFUL << CLIC_CLICCFG_NLBIT_Pos)       /*!< CLIC CFG: NLBIT Mask */

#define CLIC_CLICCFG_NMBIT_Pos          5U                                      /*!< CLIC CFG: NMBIT Position */
#define CLIC_CLICCFG_NMBIT_Msk          (0x3UL << CLIC_CLICCFG_NMBIT_Pos)       /*!< CLIC CFG: NMBIT Mask */

#define CLIC_MAX_NLBITS                 8U                                      /*!< Max nlbit of the CLICINTCTLBITS */

#define CLIC_NON_VECTOR_INTERRUPT       0x0                                     /*!< Non-Vector Interrupt Mode of CLIC */
#define CLIC_VECTOR_INTERRUPT           0x1                                     /*!< Vector Interrupt Mode of CLIC */
...
{
    CLIC->CTRL[IRQn].IE |= CLIC_INTIE_IE_Msk;
}
...
{
    CLIC->CTRL[IRQn].IE |= (CLIC_INTIE_IE_Msk | CLIC_INTIE_T_Msk);
}
...
{
    return (uint32_t)(CLIC->CTRL[IRQn].IE & CLIC_INTIE_IE_Msk);
}
...
{
    return (uint32_t)(CLIC->CTRL[IRQn].IP & CLIC_INTIP_IP_Msk);
}
...
{
    CLIC->CTRL[IRQn].IP |= CLIC_INTIP_IP_Msk;
}
...
{
    uint8_t nlbits = (CLIC->INFO & CLIC_CLICINFO_CTLBIT_Msk) >> CLIC_CLICINFO_CTLBIT_Pos;
    CLIC->CTRL[IRQn].CTL = (CLIC->CTRL[IRQn].CTL & (~CLIC_INTCFG_PRIO_Msk)) | (priority << (8 - nlbits));
}
...
{
    uint8_t nlbits = (CLIC->INFO & CLIC_CLICINFO_CTLBIT_Msk) >> CLIC_CLICINFO_CTLBIT_Pos;
    return CLIC->CTRL[IRQn].CTL >> (8 - nlbits);
}
/**
 ...
 * - \ref CLIC_GetCtrlIRQ
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_ctrl_irq(int32_t IRQn, uint8_t intctrl)
{
    CLIC->CTRL[IRQn].CTL = intctrl;
}
/**
 ...
 * - \ref CLIC_SetCtrlIRQ
 */
__ALWAYS_STATIC_INLINE uint8_t csi_vic_get_ctrl_irq(int32_t IRQn)
{
    return (CLIC->CTRL[IRQn].CTL);
}
/**
 ...
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_cfg_nlbits(uint32_t nlbits)
{
 ...
 */
__ALWAYS_STATIC_INLINE uint32_t csi_vic_get_cfg_nlbits(void)
{
 ...
 * - \ref CLIC_GetInfoNum
 */
__ALWAYS_STATIC_INLINE uint32_t csi_vic_get_info_ctlbits(void)
{
 ...
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_priority_irq(int32_t IRQn, uint8_t pri)
{
    uint8_t nlbits = csi_vic_get_cfg_nlbits();
    uint8_t intctlbits = (uint8_t)csi_vic_get_info_ctlbits();
    if (nlbits < intctlbits) 
    {
        ...
        pri = pri | mask;
        uint8_t cur_ctrl = csi_vic_get_ctrl_irq(IRQn);
        cur_ctrl = cur_ctrl >> (CLIC_MAX_NLBITS - nlbits);
        cur_ctrl = cur_ctrl << (CLIC_MAX_NLBITS - nlbits);
        csi_vic_set_ctrl_irq(IRQn, (cur_ctrl | pri));
    }
}
/**
 ...
 */
__ALWAYS_STATIC_INLINE uint8_t csi_vic_get_priority_irq(int32_t IRQn)
{
    uint8_t nlbits = csi_vic_get_cfg_nlbits();
    uint8_t intctlbits = (uint8_t)csi_vic_get_info_ctlbits();
    if (nlbits < intctlbits) {
 ...
 * - \ref CLIC_GetShvIRQ
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_shv_irq(int32_t IRQn, uint32_t shv)
{
    CLIC->CTRL[IRQn].ATTR &= ~CLIC_INTATTR_SHV_Msk;
    CLIC->CTRL[IRQn].ATTR |= (uint8_t)(shv<<CLIC_INTATTR_SHV_Pos);
}
 ...
 * - \ref CLIC_SetShvIRQ
 */
__ALWAYS_STATIC_INLINE uint32_t csi_vic_get_shv_irq(int32_t IRQn)
{
    return ((int32_t)(((CLIC->CTRL[IRQn].ATTR) & CLIC_INTATTR_SHV_Msk)>>CLIC_INTATTR_SHV_Pos));
}
/**
 ...
 * - \ref CLIC_GetTrigIRQ
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_trig_irq(int32_t IRQn, uint32_t trig)
{
    CLIC->CTRL[IRQn].ATTR &= ~CLIC_INTATTR_TRIG_Msk;
    CLIC->CTRL[IRQn].ATTR |= (uint8_t)(trig<<CLIC_INTATTR_TRIG_Pos);
}
 ...
 *     - \ref CLIC_SetTrigIRQ
 */
__ALWAYS_STATIC_INLINE uint32_t csi_vic_get_trig_irq(int32_t IRQn)
{
    return ((int32_t)(((CLIC->CTRL[IRQn].ATTR) & CLIC_INTATTR_TRIG_Msk)>>CLIC_INTATTR_TRIG_Pos));
}
 ...
 */
__ALWAYS_STATIC_INLINE void csi_vic_set_level_irq(int32_t IRQn, uint8_t lvl_abs)
{
    uint8_t nlbits = csi_vic_get_cfg_nlbits();
    uint8_t intctlbits = (uint8_t)csi_vic_get_info_ctlbits();
    ...
    cur_ctrl = cur_ctrl >> nlbits;
    csi_vic_set_ctrl_irq(IRQn, (cur_ctrl | lvl));
}
/**
 ...
 */
__ALWAYS_STATIC_INLINE uint8_t csi_vic_get_level_irq(int32_t IRQn)
{
    uint8_t nlbits = csi_vic_get_cfg_nlbits();
    uint8_t intctlbits = (uint8_t)csi_vic_get_info_ctlbits();
    ...
    uint8_t intctrl = csi_vic_get_ctrl_irq(IRQn);
...
/**
  ...
  \return          CORE timer counter value.
 */
__STATIC_INLINE uint64_t csi_coret_get_value_u64(void)
{
    return CORET->MTIME;
}
