...
extern rt_size_t at_vprintfln(device_t device, const char *format, va_list args);
...
int at_obj_exec_cmd(at_client_t client, at_response_t resp, const char *cmd_expr, ...)
{
    ...
    at_osal_mutex_take(client->lock, RT_WAITING_FOREVER);
    ...
    if (resp != RT_NULL)
    {
        if (at_osal_sem_take(client->resp_notice, resp->timeout) != RT_EOK)
        {
...
__exit:
    ...
    at_osal_mutex_release(client->lock);
...
int at_client_obj_wait_connect(at_client_t client, rt_uint32_t timeout)
{
    ...
    while (1)
    {
        ...
        resp->buf_len = 0;
        resp->line_counts = 0;
        at_device_write(client->device, 0, "AT\r\n", 4);
...
#if(defined AT_PRINT_RAW_CMD && AT_PRINT_RAW_CMD == 1U)
    at_print_raw_cmd("sendline", buf, size);
#endif

    return at_device_write(client->device, 0, buf, size);
}
...
__retry:
    if(at_device_read(client->device, 0, ch, 1) == 1)
    {
        return RT_EOK;
    }
    else
    {
        result = at_osal_sem_take(client->rx_notice, rt_tick_from_millisecond(timeout));
        if (result != RT_EOK)
        {
            return result;
        }
...
at_client_t at_client_get(const char *dev_name)
{
    ...
    {
        if (rt_strcmp(at_client_table[idx].device->name, dev_name) == 0)
        {
    ...
    while(1)
    {
        if (at_recv_readline(client) > 0)
        {
            ...
            else if (client->resp != RT_NULL)
            {
                ...
                client->resp = RT_NULL;
                at_osal_sem_release(client->resp_notice);
            }
            else
            {
...
static rt_err_t at_client_rx_ind(device_t dev, rt_size_t size)
{
    int idx = 0;
    ...
    for (idx = 0; idx < AT_CLIENT_NUM_MAX; idx++)
    {
        ...
        {
            at_osal_sem_release_isr(at_client_table[idx].rx_notice);
        }
    }
...
{
#define AT_CLIENT_LOCK_NAME            "at_cl"
#define AT_CLIENT_SEM_NAME             "at_cs"
#define AT_CLIENT_RESP_NAME            "at_cr"
#define AT_CLIENT_THREAD_NAME          "at_ct"

    int result = RT_EOK;
    static int at_client_num = 0;
    char name[RT_DEVICE_NAME_MAX];
    ...
    rt_snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", AT_CLIENT_LOCK_NAME, at_client_num);
    client->lock = at_osal_mutex_create(name, AT_IPC_FLAG_FIFO);
    ...
    rt_snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", AT_CLIENT_SEM_NAME, at_client_num);
    client->rx_notice = at_osal_sem_create(name, 0, AT_IPC_FLAG_FIFO);
    ...
    rt_snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", AT_CLIENT_RESP_NAME, at_client_num);
    client->resp_notice = at_osal_sem_create(name, 0, AT_IPC_FLAG_FIFO);
    ...
    snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", AT_CLIENT_THREAD_NAME, at_client_num);
    // rt_snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", AT_CLIENT_THREAD_NAME, at_client_num);
    client->parser = at_osal_thread_create(name,
                                     (void (*)(void *parameter))client_parser,
                                     client,
                                     1024 + 512,
                                     AT_THREAD_PRIORITY_MAX / 3 - 1,
                                     5);
...
__exit:
    if (result != RT_EOK)
    {
        if (client->lock)
        {
            at_osal_mutex_delete(client->lock);
        }

        if (client->rx_notice)
        {
            at_osal_sem_delete(client->rx_notice);
        }

        if (client->resp_notice)
        {
            at_osal_sem_delete(client->resp_notice);
        }

        if (client->device)
        {
            at_device_close(client->device);
        }
...
int at_client_init(const char *dev_name,  rt_size_t recv_bufsz)
{
    ...
    client->device = at_device_find(dev_name);
    if (client->device)
    {
        // RT_ASSERT(client->device->type == AT_Device_Class_Char);

        /* using DMA mode first */
        open_result = at_device_open(client->device, AT_DEVICE_OFLAG_RDWR | AT_DEVICE_FLAG_DMA_RX);
        /* using interrupt mode when DMA mode not supported */
        if (open_result == RT_EIO)
        {
            open_result = at_device_open(client->device, AT_DEVICE_OFLAG_RDWR | AT_DEVICE_FLAG_INT_RX);
        }
        RT_ASSERT(open_result == RT_EOK);

        at_device_set_rx_indicate(client->device, at_client_rx_ind);
    }
    else
    {
...
__exit:
    if (result == RT_EOK)
    {
        ...
        at_osal_thread_startup(client->parser);
...
void at_client_set_flag(at_client_t client, unsigned int pos, bool link_mux)
{
    if(client != RT_NULL)
    {
        if(link_mux)
...
bool at_client_get_flag(at_client_t client, unsigned int pos)
{
    if(client != RT_NULL)
    {
        return ((client->flag & (0x1 << pos)) != 0);
    }
    return false;
}
