...
//
  ...
  if((heth->Init).AutoNegotiation != ETH_AUTONEGOTIATION_DISABLE)
  {
    ...
    if((phyreg & PHY_SPEED_STATUS) == PHY_SPEED_STATUS_10M)
    {
...
static void ETH_MACDMAConfig(ETH_HandleTypeDef *heth, uint32_t err)
{
  ...
  macinit.Jabber = ETH_JABBER_DISABLE;
  ...
  macinit.AutomaticPadCRCStrip = ETH_AUTOMATICPADCRCSTRIP_ENABLE;
  macinit.BackOffLimit = ETH_BACKOFFLIMIT_10;
  macinit.DeferralCheck = ETH_DEFFERRALCHECK_DISABLE;
  macinit.ReceiveAll = ETH_RECEIVEAll_DISABLE;
  macinit.SourceAddrFilter = ETH_SOURCEADDRFILTER_NORMAL_ENABLE;
  ...
  macinit.PauseTime = 0xFFFFU;
  macinit.ZeroQuantaPause = ETH_ZEROQUANTAPAUSE_ENABLE;
  macinit.PauseLowThreshold = ETH_PAUSELOWTHRESHOLD_MINUS4;
  macinit.UnicastPauseFrameDetect = ETH_UNICASTPAUSEFRAMEDETECT_DISABLE;
  macinit.ReceiveFlowControl = ETH_RECEIVEFLOWCONTROL_ENABLE;
  macinit.TransmitFlowControl = ETH_TRANSMITFLOWCONTROL_ENABLE;
  ...
  tmpreg1 |= ETH_FRAME_BURST_ENABLE;
  ...
  (heth->Instance)->MACFFR = (uint32_t)(macinit.ReceiveAll |
   ...
   (heth->Instance)->MACVLANTR = (uint32_t)(macinit.VLANTagComparison |
    ...
    dmainit.ForwardErrorFrames = ETH_FORWARDERRORFRAMES_ENABLE;
    dmainit.ForwardUndersizedGoodFrames = ETH_FORWARDUNDERSIZEDGOODFRAMES_DISABLE;
    dmainit.ReceiveThresholdControl = ETH_RECEIVEDTHRESHOLDCONTROL_128BYTES;
    dmainit.SecondFrameOperate = ETH_SECONDFRAMEOPERARTE_ENABLE;
    dmainit.AddressAlignedBeats = ETH_ADDRESSALIGNEDBEATS_DISABLE;
    dmainit.FixedBurst = ETH_FIXEDBURST_DISABLE;
    dmainit.RxDMABurstLength = ETH_RXDMABURSTLENGTH_4BEAT;
    dmainit.TxDMABurstLength = ETH_TXDMABURSTLENGTH_4BEAT;
    dmainit.EnhancedDescriptorFormat = ETH_DMAENHANCEDDESCRIPTOR_DISABLE;
    dmainit.DescriptorSkipLength = 0x04U;
    ...
    (heth->Instance)->DMABMR = (uint32_t)(dmainit.AddressAlignedBeats |
                                          ...
                                          dmainit.DMAArbitration); /* Enable use of separate PBL for Rx and Tx */
     ...
     if((heth->Init).RxMode == ETH_RXINTERRUPT_MODE)
     {
       ...
       __HAL_ETH_DMA_ENABLE_IT((heth), ETH_DMA_IT_NIS | ETH_DMA_IT_AIS | ETH_DMA_IT_FBE |
               ETH_DMA_IT_RPS | ETH_DMA_IT_RBU | ETH_DMA_IT_R | ETH_DMA_IT_TU |
               ETH_DMA_IT_TPS | ETH_DMA_IT_T);
     }
...
static void ETH_Delay(uint32_t mdelay)
{
  __IO uint32_t Delay = mdelay * (GetSystemCoreClock() / 8U / 1000U);
  do
  {
    __NOP();
  }
