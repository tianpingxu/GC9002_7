...
static rt_mutex_t sal_core_lock;
...
int sal_init(void)
{
    ...
    sal_core_lock = at_osal_mutex_create("sal_lock", RT_IPC_FLAG_FIFO);
...
static void sal_lock(void)
{
    ...
    result = at_osal_mutex_take(sal_core_lock, RT_WAITING_FOREVER);
    if (result != RT_EOK)
    {
        RT_ASSERT(0);
    }
...
{
    at_osal_mutex_release(sal_core_lock);
}
