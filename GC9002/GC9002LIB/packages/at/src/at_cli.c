...
#if(defined AT_USING_CLI && AT_USING_CLI == 1U)

#define AT_CLI_FIFO_SIZE                      256

static rt_sem_t console_rx_notice;
static rt_ringbuffer_t console_rx_fifo = RT_NULL;
static rt_err_t (*odev_rx_ind)(device_t dev, rt_size_t size) = RT_NULL;

#ifdef AT_USING_CLIENT
static rt_sem_t client_rx_notice;
static rt_ringbuffer_t client_rx_fifo = RT_NULL;
#endif 
...
static char console_getchar(void)
{
    ...
    at_osal_sem_take(console_rx_notice, RT_WAITING_FOREVER);
    rt_ringbuffer_getchar(console_rx_fifo, (rt_uint8_t *)&ch);

    return ch;
}

static rt_err_t console_getchar_rx_ind(device_t dev, rt_size_t size)
{
    uint8_t ch;
    rt_size_t i;
    ...
    {
        /* read a char */
        if (at_device_read(dev, 0, &ch, 1))
        {
            rt_ringbuffer_put_force(console_rx_fifo, &ch, 1);
            at_osal_sem_release_isr(console_rx_notice);
        }
    }
...
void at_cli_init(void)
{
    ...
    at_osal_sem_init(&console_rx_notice, "cli_c", 0, AT_IPC_FLAG_FIFO);
    ...
    #if(defined AT_USING_CLI && AT_USING_CLI == 1U)
    // #error "at_console_get_device() is unrealized"
    #endif
    console = at_console_get_device();
    if (console)
    {
        /* backup RX indicate */
        odev_rx_ind = console->rx_indicate;
        at_device_set_rx_indicate(console, console_getchar_rx_ind);
    }
...
void at_cli_deinit(void)
{
    ...
    {
        /* restore RX indicate */
        at_device_set_rx_indicate(console, odev_rx_ind);
    }
    rt_hw_interrupt_enable(int_lvl);

    at_osal_sem_detach(console_rx_notice);
    rt_ringbuffer_destroy(console_rx_fifo);
}

#if(defined AT_USING_SERVER && AT_USING_SERVER == 1U)
...
static char client_getchar(void)
{
    ...
    at_osal_sem_take(client_rx_notice, RT_WAITING_FOREVER);
...
static rt_err_t client_getchar_rx_ind(device_t dev, rt_size_t size)
{
    uint8_t ch;
    rt_size_t i;
    ...
    for (i = 0; i < size; i++)
    {
        ...
        if (at_device_read(dev, 0, &ch, 1))
        {
            ...
            at_osal_sem_release(client_rx_notice);
        }
    }
...
#define DELECT_KEY              0x7F
    ...
    static rt_err_t (*client_odev_rx_ind)(device_t dev, rt_size_t size) = RT_NULL;
    rt_base_t int_lvl;
    ...
    if (client)
    {
        ...
        /* backup client device RX indicate */
        {
            ...
            at_device_set_rx_indicate(client->device, client_getchar_rx_ind);
            rt_hw_interrupt_enable(int_lvl);
        }

        at_osal_sem_init(&client_rx_notice, "cli_r", 0, AT_IPC_FLAG_FIFO);
        client_rx_fifo = ringbuffer_create(AT_CLI_FIFO_SIZE);

        at_client = at_osal_thread_create("at_cli", at_client_entry, RT_NULL, 512, 8, 8);
        if (client_rx_fifo && at_client)
        {
            ...
            at_osal_thread_startup(at_client);
            /* process user input */
            ...
            {
                int_lvl = rt_hw_interrupt_disable();
                at_device_set_rx_indicate(client->device, client_odev_rx_ind);
                rt_hw_interrupt_enable(int_lvl);
            }

            at_osal_thread_delete(at_client);
            at_osal_sem_detach(client_rx_notice);
