...
//#define NULL ((void *)0)
  ...
  rs->alpha_to = (data_t *)qr_malloc(sizeof(data_t)*(rs->nn+1));
  if(rs->alpha_to == NULL){
    qr_free(rs);
    rs = NULL;
    goto done;
  }
  rs->index_of = (data_t *)qr_malloc(sizeof(data_t)*(rs->nn+1));
  if(rs->index_of == NULL){
    qr_free(rs->alpha_to);
    qr_free(rs);
    rs = NULL;
  ...
  rs->genpoly = (data_t *)qr_malloc(sizeof(data_t)*(nroots+1));
  if(rs->genpoly == NULL){
    qr_free(rs->alpha_to);
    qr_free(rs->index_of);
    qr_free(rs);
