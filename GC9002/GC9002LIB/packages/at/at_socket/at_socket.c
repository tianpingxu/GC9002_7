...
static struct at_socket *alloc_socket_by_device(struct at_device *device, enum at_socket_type type)
{
    ...
    char name[RT_DEVICE_NAME_MAX] = {0};
    int idx = 0;

    if (at_slock == RT_NULL)
    {
        /* create AT socket lock */
        at_slock = at_osal_mutex_create("at_slock", RT_IPC_FLAG_FIFO);
        if (at_slock == RT_NULL)
        {
    ...
    at_osal_mutex_take(at_slock, RT_WAITING_FOREVER);
    ...
    rt_snprintf(name, RT_DEVICE_NAME_MAX, "%s%d", "at_skt", idx);
    /* create AT socket receive mailbox */
    if ((sock->recv_notice = at_osal_sem_create(name, 0, RT_IPC_FLAG_FIFO)) == RT_NULL)
    {
    ...
    if((sock->recv_lock = at_osal_mutex_create(name, RT_IPC_FLAG_FIFO)) == RT_NULL)
    {
        LOG_E("No memory for socket receive mutex create.");
        at_osal_sem_delete(sock->recv_notice);
        goto __err;
    }

    at_osal_mutex_release(at_slock);
    return sock;
...
static struct at_socket *alloc_socket(enum at_socket_type type)
{
    ...
    // device = at_device_get_by_name(AT_DEVICE_NAMETYPE_CLIENT, AT_SAMPLE_DEIVCE_NAME);
...
static int free_socket(struct at_socket *sock)
{
    ...
    if (sock->recv_lock)
    {
        at_osal_mutex_delete(sock->recv_lock);
    }
...
static void at_recv_notice_cb(struct at_socket *sock, at_socket_evt_t event, const char *buff, size_t bfsz)
{
    ...
    at_osal_mutex_take(sock->recv_lock, RT_WAITING_FOREVER);
    at_recvpkt_put(&(sock->recvpkt_list), buff, bfsz);
    at_osal_mutex_release(sock->recv_lock);

    at_osal_sem_release(sock->recv_notice);
...
int at_recvfrom(int socket, void *mem, size_t len, int flags, struct sockaddr *from, socklen_t *fromlen)
{
    ...
    while (1)
    {
        ...
        if (at_osal_sem_take(sock->recv_notice, timeout) < 0)
        {
