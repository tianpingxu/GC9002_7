...
static QRinput_List *QRinput_List_newEntry(QRencodeMode mode, int size, const unsigned char *data)
{
	...
	entry = (QRinput_List *)qr_malloc(sizeof(QRinput_List));
	if(entry == NULL) return NULL;

	entry->mode = mode;
	entry->size = size;
	if(size > 0) {
		entry->data = (unsigned char *)qr_malloc(size);
		if(entry->data == NULL) {
			qr_free(entry);
			return NULL;
		}
...
static QRinput_List *QRinput_List_dup(QRinput_List *entry)
{
	...
	n = (QRinput_List *)qr_malloc(sizeof(QRinput_List));
	if(n == NULL) return NULL;

	n->mode = entry->mode;
	n->size = entry->size;
	n->data = (unsigned char *)qr_malloc(n->size);
	if(n->data == NULL) {
		qr_free(n);
		return NULL;
	}
...
QRinput *QRinput_new2(int version, QRecLevel level)
{
	...
	input = (QRinput *)qr_malloc(sizeof(QRinput));
...
static int QRinput_appendPaddingBit(BitStream *bstream, QRinput *input)
{
	...
	if(padlen > 0) {
		padbuf = (unsigned char *)qr_malloc(padlen);
		if(padbuf == NULL) {
			ret = -1;
			goto DONE;
		}
...
static QRinput_InputList *QRinput_InputList_newEntry(QRinput *input)
{
	...
	entry = (QRinput_InputList *)qr_malloc(sizeof(QRinput_InputList));
	if(entry == NULL) return NULL;
...
QRinput_Struct *QRinput_Struct_new(void)
{
	...
	s = (QRinput_Struct *)qr_malloc(sizeof(QRinput_Struct));
...
static int QRinput_List_shrinkEntry(QRinput_List *entry, int bytes)
{
	...
	data = (unsigned char *)qr_malloc(bytes);
