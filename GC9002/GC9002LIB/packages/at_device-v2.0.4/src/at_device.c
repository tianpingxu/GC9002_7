    ...
    rt_slist_for_each(node, &at_device_list)
    {
        ...
        if (device)
        {
                ...
                (rt_strncmp(device->name, name, rt_strlen(name)) == 0))
            {
                ...
                (rt_strncmp(device->client->device->name, name, rt_strlen(name)) == 0))
            {
                rt_hw_interrupt_enable(level);
                return device;
            }
 ...
 * network
 * @return != RT_NULL: network interface device object
 *            RT_NULL: get failed
 */
 ...
 * @param buff the result of return
...
int at_device_control(struct at_device *device, int cmd, char *arg, char *buff, int size)
{
    ...
    {
        return device->class->device_ops->control(device, cmd, arg, buff, size);
    }
    else
    {
                        ...
                        const char *at_client_name, uint16_t class_id, void *user_data)
{
    ...
    rt_snprintf(name, RT_DEVICE_NAME_MAX, "at_se%d", device_counts++);
    device->socket_event = at_osal_event_create(name, RT_IPC_FLAG_FIFO);
...
{
    if (device->command_groups_count == AT_MAX_COMMAND_GROUPS)
    {
        LOG_E("command groups limit exceeded\n");
        return;
    }
    command_group_t* next = device->command_groups + device->command_groups_count;
    next->groupname      = groupname;
    next->commands       = desc;
    next->commands_count = ndesc;
    next->context        = group_ctx;

    device->command_groups_count++;
}
