...
void DISPLAY_SetUpdateRange(uint16_t startX, uint16_t startY, uint16_t width, uint16_t height)
{
	DISPLAY->timing_param0 &= 0x0000FFFF;
	DISPLAY->timing_param0 |= (width << 16);
	
	DISPLAY->timing_param2 &= 0x0000FFFF;
	DISPLAY->timing_param2 |= (height << 16);
	
	DISPLAY->col_update_area = ((startX + width - 1) << 16) + startX;

    DISPLAY->row_update_area = ((startY + height - 1) << 16) + startY;
	
	DISPLAY->total_byte_cnt = width * height * 2;
	
	AHMI_InitTypeDef AHMI_InitStruct;
    AHMI_StructInit_UpdateRange(&AHMI_InitStruct);
...
/**
	*	@brief get primary frame base address
	*	@param none
	*	@retval	current primary frame base address
	*/
uint32_t DISPLAY_GetPriAddress(void)
{
	return DISPLAY->base_addr;
}
