...
{
	printf("configure_done\n");
    /* setup first out ep read transfer */
    usbd_ep_start_read(CDC_OUT_EP, read_buffer, 64);
	connected =true;
...
{
    USB_LOG_RAW("actual in len:%d,ep: %04x\r\n", nbytes,ep);
...
bool cdc_acm_data_send_with_dtr_test(void)
{
	...
	{
		ep_tx_busy_flag = true;
		usbd_ep_start_write(CDC_IN_EP, data, 5*1024);
	...
	double speed = 5.0*1000*count/(t2-t1);
