...
#define MUSB_TXFIFOSZ_OFFSET  0x1C
#define MUSB_RXFIFOSZ_OFFSET  0x1D
#define MUSB_TXFIFOADD_OFFSET 0x1E
#define MUSB_RXFIFOADD_OFFSET 0x1F
...
int usbd_ep_open(const struct usbd_endpoint_cfg *ep_cfg)
{
    ...
    if (USB_EP_DIR_IS_OUT(ep_cfg->ep_addr)) {
		...
		HWREGH(USB_BASE + MUSB_TXIE1_OFFSET) |= (1 << ep_idx);
...
int usbd_ep_start_read(const uint8_t ep, uint8_t *data, uint32_t data_len)
{
    ...
    if (ep_idx == 0) {
        ...
        HWREGH(USB_BASE + MUSB_RXIE1_OFFSET) |= (1 << ep_idx);
    }
    musb_set_active_ep(old_ep_idx);
    return 0;
}
    ...
    switch (usb_ep0_state) {
		...
		//printf("USB_EP0_STATE_SETUP\n");
            if (ep0_status & USB_CSRL0_RXRDY) {
                read_count = HWREGH(USB_BASE + MUSB_IND_RXCOUNT1_OFFSET);
		...
		//printf("USB_EP0_STATE_IN_STATUS\n");
...
__attribute__ ((interrupt ("machine"))) void OTG_IRQHandler(void)
{
    ...
    uint32_t is;
    uint32_t txis ,txie = 0, txis_ex = 0;
    uint32_t rxis , rxie = 0;
    uint8_t old_ep_idx;
    uint8_t ep_idx;
    uint16_t write_count, read_count;

    is = HWREGB(USB_BASE + MUSB_IS_OFFSET);
    txis = HWREGH(USB_BASE + MUSB_TXIS1_OFFSET);
    rxis = HWREGH(USB_BASE + MUSB_RXIS1_OFFSET);
 ...
 //   HWREGB(USB_BASE + MUSB_IS_OFFSET) &= ~is;
	...
	uint8_t index = HWREGB(USB_BASE + MUSB_EPIDX_OFFSET);
	uint8_t ie = HWREGB(USB_BASE + MUSB_IE_OFFSET);
//	printf("txis:%4x,txiE:%4x\n",txis,HWREGB(USB_BASE + MUSB_TXIE1_OFFSET));
    ...
    if (is & USB_IS_RESUME) {
		printf("usb is RESUME\n");
    }

    if (is & USB_IS_SUSPEND) {
		printf("usb is SUSPEND\n");
    }

    txis &= HWREGH(USB_BASE + MUSB_TXIE1_OFFSET);
    /* Handle EP0 interrupt */
    if (txis & USB_TXIE_EP0) {
		//printf("handle ep0\n");
        HWREGB(USB_BASE + MUSB_TXIS1_OFFSET) = USB_TXIE_EP0;
		 HWREGB(USB_BASE + MUSB_TXIS2_OFFSET) = 0 ;
        musb_set_active_ep(0);
        handle_ep0();
        txis &= ~USB_TXIE_EP0;
    }
    ...
    while (txis) {
        if (txis & (1 << ep_idx)) {
            musb_set_active_ep(ep_idx);
            HWREGH(USB_BASE + MUSB_TXIS1_OFFSET) = (1 << ep_idx);
...
//				printf("@");
    ...
    rxis &= HWREGH(USB_BASE + MUSB_RXIE1_OFFSET);
    ep_idx = 1;
    while (rxis) {
        if (rxis & (1 << ep_idx)) {
            musb_set_active_ep(ep_idx);
            HWREGH(USB_BASE + MUSB_RXIS1_OFFSET) = (1 << ep_idx);
